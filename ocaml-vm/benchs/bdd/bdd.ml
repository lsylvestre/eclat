...
let main () =
  let n =
    (*if Array.length Sys.argv >= 2 then int_of_string Sys.argv.(1) else*) 2 in
  let ntests =
    (*if Array.length Sys.argv >= 3 then int_of_string Sys.argv.(2) else*) 2 in
  let bdd = hwb n in
