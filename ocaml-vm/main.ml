...
let target_globals_rom_size = 4096 ;;
...
let () =
  ...
  Arg.parse [
    ...
    ("-ml-syntax", Arg.Set ml_syntax_flag,
...
let rec _add_val oc i v =
  (let open Value in
        match v with
        | Int n -> 
           Printf.fprintf oc "print_int %d; print_string \":\";" !ri;
           Printf.fprintf oc "print (val_long(%d)); print_newline ();\n" n;
          !ri
        | Block (tag,vs) ->
           incr ri;
           let j = !ri in
           Printf.fprintf oc "print_int %d; print_string \":\"; print (val_ptr(%d));\nprint_newline ();\n" i j;
           let len = Array.length vs+1 in
           Printf.fprintf oc "print_int %d; print_string \":\"; print (val_long(make_header(%d,%d)));\nprint_newline ();\n" i tag (len-1);
           Array.iter (fun v ->
              let next_i = _add_val oc !ri v in
              ri := next_i;
              (* Printf.fprintf oc "print_int %d; print_string \":\"; print (%s);\nprint_newline ();\n" !next_i !next_i (!next_i+1) res*)
            ) vs;
              !ri
        (*| String s -> 
            let vs = List.map (fun c -> Int (Char.code c)) (charlist_of_string s) in
            _add_val oc i (Block(252,Array.of_list vs))*)
        | _ -> 0 (* failwith "unsupported value type"*)
        );;

let _write_data oc data =
  Printf.fprintf oc "let init_data () =\n";
  let i = ref 0 in
  Printf.fprintf oc "let x1 = global_start + 256 in\n";
  Array.iter (fun v ->
      if !i >= 12 then
        (let n = !i in
        Printf.fprintf oc "(* ADD GLOBAL %d *)\n" n;
        let _ = _add_val oc !ri v in
        ());
        incr i;
    ) data ;;
...
let write_code2 ~version ~prim oc oc2 code =
  let tmp_code_path = "tmp/code.txt" in
  let oc_tmp = open_out tmp_code_path in
  Code.write version oc_tmp code;
  ...
  let to_string i =
    ...
    | C_CALL(narg, idx) ->
        let num = Hashtbl.find primitives prim.(idx) in
