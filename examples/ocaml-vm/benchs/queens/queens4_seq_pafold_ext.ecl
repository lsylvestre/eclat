(* make SRC="benchs/queens/queens4.ml"  FLAGS+="-ocaml" BCOPT="-load-code -load-data -custom caml_queens" CUSTOM=ocaml-vm/benchs/queens/queens4_seq_pafold_ext.ecl  byte
   ...
   if safe_all (col,i,chess) then 
     ( let chess = vect_copy_with(chess,col,i) in
       let nb2 = if col+1 = n then ( 
           ...
           let (next_link,st) = alloc_block(1,2,st) in
...
let caml_queens ((v,_),st) = print_string "hello!!!";
  ...
  let (pc,acc,sp, (env, extra_args, trap_sp), others) = st in
  let st = (pc,link,sp, (env, extra_args, trap_sp), others) in
  set(ref_link,0,link);
  set(ref_st,0,st);
