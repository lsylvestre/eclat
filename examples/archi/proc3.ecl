(* 
    ...
    $ ./eclat ../examples/archi/alu.ecl ../examples/archi/proc3.ecl
    $ make simul NS=100
...
let execute(de_ex, pc) =
  ...
  ((next_ex_mem_rd, next_ex_mem_res), pc, br_taken, rdy) ;;
...
(* ============== Instruction Fetch ============== *)
let ifetch(rdy, (next_br_taken, next_pc, (code,pc)), if_de) =
  if rdy then
    let next_if_de_instr = vect_nth(code,pc) in
    ((next_if_de_instr, ()), (if next_br_taken then next_pc else pc + 1))
  else (if_de, pc) ;;


let config0 = (vect_create<32>(0), (0,0), (0,0),
               (Alu(Add()),0,0,0), (((Alu(Add()),Rs2Imm()),0,0,0),()), 0,false, true) ;;

let flush(reg_file, pc) = 
  (reg_file, (0,0), (0,0), (Alu(Add()),0,0,0), (((Alu(Add()),Rs2Imm()),0,0,0),()), pc, false, true) ;;
 ...
 * ================================ *)
let cpu_step s =
     let (reg_file, _, _, _, _, pc, br_taken, _) = s in
     let (reg_file, mem_wb, ex_mem, de_ex, if_de, pc, br_taken, rdy) = 
       if br_taken then flush(reg_file, pc) else s 
     in
     let next_reg_file           = write_back (rdy, mem_wb, reg_file) in
     let next_mem_wb             = mem        (rdy, ex_mem, mem_wb)   in
     let (next_ex_mem, next_pc,
          next_br_taken,
          next_rdy)              = execute    (de_ex, pc)             in
     let next_de_ex              = decod      (next_rdy, (if_de, reg_file), de_ex)  in
     let (next_if_de, next_pc)   = ifetch     (next_rdy,(next_br_taken, next_pc, (code,pc)),if_de) in
     (next_reg_file, next_mem_wb, next_ex_mem, next_de_ex, next_if_de, next_pc, next_br_taken, next_rdy)
;;
