...
let rec id_tau e t =
  ...
  | E_app(e1, e2) ->
      let x = gensym () in
      E_letIn(P_var x, Types.new_ty_unknown(), e, ty_annot ~ty:t (E_var x))
  | E_tuple(es) ->
      (match t with
  ...
  | E_par(es) ->
      ...
      | _ -> assert false)
  | E_letIn(p, ty, e1, e2) -> 
      E_letIn(p,ty, e1, id_tau e2 t)
  | E_fun(p, sigty, e1) ->
      (match t with
       | Ty_fun(t1,_,t2) ->
           E_fun(p,sigty,E_letIn(p,t1, (Pattern.pat2exp(p)),ty_annot ~ty:(Ty_base t2) e1))
       | _ -> assert false)
  | E_fix(f,(p,sigty,e1)) ->
      (match t with
       | Ty_fun(t1,_,t2) ->
           E_fix(f,(p,sigty,E_letIn(p,t1, (Pattern.pat2exp(p)),ty_annot ~ty:(Ty_base t2) e1)))
       | _ -> assert false)
  | E_reg((p,tyB,e),e0,l) ->
      E_reg((p,tyB,id_tau e t),id_tau e0 t,l)
  | E_exec(e1,e0,eo,l) ->
      (match t with
    ...
    | v::vs' ->
       ...
       (let e',mu' = eval_prog mu ef v in
        if evaluated e' then (
          ...
          fprintf std_formatter "%a@]@," (emph red) "(running)";
          eval_aux (i+1) n mu' (E_fun(P_var "_",(Types.new_ty_unknown(),Types.new_tyB_unknown()),e')) vs'
        )
    )) in 
    eval_aux 0 n mu (* (Ast_undecorated.remove_deco*) main vs
