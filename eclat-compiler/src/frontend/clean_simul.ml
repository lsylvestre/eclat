    ...
    | E_app(e1,e2) ->
        let opt = match un_annot e1 with
                  | E_const(Op(Runtime(Print | Print_string | Print_int | Print_newline))) ->
                       ...
                       if no_print then (Some (E_letIn(P_var x,Ty_base TyB_unit,e2,E_const(Unit)))) else None
                  | E_const(Op(Runtime(Assert))) ->
                       let x = gensym ~prefix:"tmp" () in
                       if no_assert then Some (E_letIn(P_var x,Ty_base TyB_unit,e2,E_const(Unit))) else None
                  | _ -> None in
        (match opt with
