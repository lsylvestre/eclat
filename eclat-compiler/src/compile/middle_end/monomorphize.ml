...
let eq_tys t1 t2 =
  ...
  and compare_ty t1 t2 =
    ...
    | T_var ({contents=Unknown n}),
      T_var ({contents=Unknown m}) -> () (* will be unified in the generated code *)
...
let monomorphize_exp e =
  let rec aux ds e =
    ...
    | E_letIn(P_var f,E_fix(_,(p,e1)),e2) -> 
        ...
        E_letIn(P_var f,v,aux ((f,v)::ds) e2)
    | E_app(E_var f,arg) ->
       let lty = Hashtbl.find_all Typing.signatures f in
       let ty,lty' = match lty with
       | [] -> assert false
       | ty::lty' -> ty,lty' in
       if List.for_all (eq_tys ty) lty' then e else
