...
let rec remove_deco e =
  ...
  (* ************************************* *)
  | E_letIn(p,sigty,e1,e2) ->
      E_letIn(remove_deco_pat p, sigty, remove_deco e1, remove_deco e2)
  | E_fun(p,sigty,e1) ->
      E_fun(remove_deco_pat p, sigty, remove_deco e1)
  | E_fix(f,(p,sigty,e1)) ->
      E_fix(f,(remove_deco_pat p, sigty, remove_deco e1))
  | E_reg((p,tyB,e1),e0,l) ->
      E_reg((remove_deco_pat p, tyB, remove_deco e1),remove_deco e0,l)
  ...
  | E_generate((p,sigty,e1),e2,sz1,sz2,loc) ->
      E_generate((remove_deco_pat p,sigty,remove_deco e1),remove_deco e2,sz1,sz2,loc)
  (* ************************************* *)
