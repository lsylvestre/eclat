  ...
  let rec deadcode_elim = function
    | E_letIn(P_var f,tyf,(E_fix(h,(p,(ty,tyB),e1)) as phi),e2) ->
        if (f <> h) then deadcode_elim (E_letIn(P_var f,(*tyf*)Types.new_ty_unknown(),
                                           (E_fix(f,(p,(Types.new_ty_unknown(),Types.new_tyB_unknown())(*(ty,tyB)*), Ast_subst.subst_e h (E_var f) e1))),e2)) else
...
let rec map_under_exec_and_par e =
  ...
  let rec aux env e =
    ...
    | E_letIn(P_var f,ty,(E_fix(g,(p,(ty1,tyB2),e1))),e2) ->
        ...
        (** todo: avoid fresh unknowns and loose of type annotation. 
            The problem is about instantiation of type variables in presence 
            of polymorphic recursive function (which should be monomorphized before) **)
